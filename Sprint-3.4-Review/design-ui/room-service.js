...
(function (global) {
  ...
  function setReady(roomId, playerId, ready) {
    ...
    return db().runTransaction(function (tx) {
      ...
      // comment). This is a fire-and-forget follow-up: it never blocks
      // or fails setReady()'s own already-committed success, matching
      // the same fail-open shape as syncCurrentRoomOnProfile above.
      maybeStartMatch(roomId, room);
      return room;
    });
  }

  function maybeStartMatch(roomId, room) {
    if (!room || room.status !== "waiting") return; // already in_game/closed
    var players = room.players || [];
    var readyPlayers = room.readyPlayers || [];
    ...
    global.MatchService.startMatch(roomId).catch(function (err) {
      console.error("[RoomService] MatchService.startMatch failed (non-fatal to setReady itself):", err);
    });
  }
