...
- **Purpose (as shipped today):** the match document `MatchService.startMatch()` creates the moment every player in a room readies up. Holds only what's needed to prove "a match exists and these players are in it" — nothing about bidding, dealing, or play yet.
...
- **Fields (current, actual):**
  ...
  - `players: string[]` — copied from the room's `players` array at the moment of creation. Flat `uid` array, **not** the speculative `[{ uid, name, isAI, seat }]` object-array shape below — no seat concept exists in the shipped schema yet, matching the same simplification `rooms/{roomId}` already made.
  - `status: "starting"` — the only value that exists today; `MatchService`'s gameplay methods (`submitBid`/`playCard`/etc.) that would eventually transition it further are still stubs.
  - `createdAt: timestamp`.
  ...
  - `turn: string` — one of `players[]`; defaults to the same value as `dealer`. **Has no real meaning yet** — turn order is determined by bidding/estimation, which don't exist yet. A placeholder, not a claim that turn order is implemented.
  - `gameState: { initialized: false, todo: string }` — an explicit TODO placeholder, **not** real dealt hands. `Dealer.dealHands()` (`design-ui/engine/dealer.js`) depends on a global `Deck` object that does not exist anywhere in this repository (`deck.js` was never delivered — only referenced in `dealer.js`'s own header comment). See `docs/implementation/MatchInitialization.md` for the full finding.
- **Fields NOT yet implemented — DO NOT implement against these without re-checking this doc first:** `mode`, `scoringMode`, `dealerId` (use `dealer`), `turnId` (use `turn`), `round` (use `currentRound`, a bare number, not an object), `hands`, `dealState`, `playState`, `biddingState`, `matchScores`, `roundHistory`, `winnerId`, `startedAt` (use `createdAt`), `updatedAt`. All of these belong to the speculative future-gameplay design retained below for planning purposes only.
- **Relationships:** `players[]` → `players/{uid}`; `roomId` → `rooms/{roomId}`.
- **Read frequency (as shipped):** Low — created once per room, read via `loadMatch`/`subscribeToMatch` only by the Match placeholder screen today; nothing polls it repeatedly yet.
- **Write frequency (as shipped):** Exactly one write per room (the creation write inside `startMatch()`'s transaction) — no further writes exist yet, since no gameplay method is implemented.
- **Security requirements (as actually deployed — see `firestore.rules` and `docs/implementation/MatchInitialization.md`):** `get` restricted to a player listed in the match's own `players[]` (not a global authenticated read, unlike `rooms/{roomId}` — see `SecurityArchitecture.md`'s note on why); `list` always denied; `create` requires the acting user be one of the new document's own `players[]`, field-whitelisted to the actual shipped shape, `status == "starting"`, `currentRound == 1`, `dealer`/`turn` both in `players[]`; `update` **denied outright** (`allow update: if false`) — correct for the actually-shipped code, since no gameplay write path exists yet; `delete` always denied.
