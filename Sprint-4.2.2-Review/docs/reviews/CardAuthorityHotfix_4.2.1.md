> **UPDATE — Sprint 4.2.2 (Atomic Card Turn Progression & Card-Log Desync Hardening):** a direct review of this sprint's shipped code found three remaining correctness defects — (1) `submitCard()` appended the card but never updated `matches/{matchId}.turn`, so the next player was rejected as "not your turn" even though it genuinely was; (2) `applyRemoteCard()` silently skipped a `MALFORMED_ENTRY` item instead of treating it as a desync; (3) the local-echo check compared only seat identity, not card identity, so a different card from the same seat could be silently accepted as an echo. **All three are now fixed** — see `docs/reviews/CardTurnProgressionHotfix_4.2.2.md` for the full account. This document is left otherwise unedited, as the historical record of Sprint 4.2.1's own original (now-superseded) state — per this project's "correct forward, don't rewrite history" convention.
