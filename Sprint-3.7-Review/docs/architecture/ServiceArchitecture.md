...
> **Re-synced in Sprint 3.4, hardened in Sprint 3.4.1, made production-ready for real-time sync in Sprint 3.7** (previous state: Sprint 2.7 speculative skeleton, all methods throwing `Not implemented`). See `docs/implementation/MatchInitialization.md` for the Sprint 3.4/3.4.1 report and `docs/architecture/MatchSynchronization.md` for the Sprint 3.7 report. Depends on `SessionService` through its existing public API only (Sprint 3.4.1 removed the last direct `PlayerService` call — see below); is depended on by `RoomService` (a new, one-directional `RoomService → MatchService` edge — see `room-service.js`'s header comment — introduces no circular dependency, since `MatchService` never calls back into `RoomService`) and, new in Sprint 3.7, by `GameSession` (`design-ui/engine/session.js`), which now calls `MatchService.subscribeToMatch()` to consume live match updates — a one-directional `GameSession → MatchService` edge; `MatchService` still has no dependency on the engine layer at all, in either direction.
...
- `subscribeToMatch(matchId, callback) → unsubscribe` — live `onSnapshot` listener; delivers `(data, null)` or `(data-or-null, err)` to the callback rather than throwing, mirroring `PlayerService.subscribeToPlayerProfile`'s established pattern. (Named `subscribeToMatch`, not the brief's literal `subscribe(matchId)`, for naming consistency with every other service's subscribe method in this codebase — restated again in Sprint 3.7's brief and, again, kept for the same reason.) **Made production-ready in Sprint 3.7 (Real-Time Match Synchronization)** — see `docs/architecture/MatchSynchronization.md` for the full design. Now **ref-counted per `matchId`**: any number of local callers subscribing to the same `matchId` share exactly ONE real Firestore listener, torn down only once the last one unsubscribes (no duplicated listeners, no leak). Carries an **ordering guard** (a document with a numeric `version` field is only published if strictly newer than the last one seen — armed but dormant today, since no shipped write path sets `version` yet) and a **duplicate-content guard** (an identical re-delivery is never re-published — the concrete "ignore local duplicate updates, prevent infinite loops" mechanism). On disconnect, delivers the last known good data *alongside* the error (never `null`) and **automatically reconnects** with exponential backoff (250ms→4s cap) — the caller never needs to notice a disconnect and re-subscribe.
