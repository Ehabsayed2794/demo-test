...
>
> **Sprint 3.7 (Real-Time Match Synchronization)** also did not move this slice further along the diagram — `DEALING` onward remains not implemented, and no gameplay write path exists (`firestore.rules`' `matches/{matchId}` block still correctly has `allow update: if false`). What changed: the read side of this lifecycle is now real-time rather than one-shot. Any seated client that calls `MatchService.subscribeToMatch(matchId, ...)` (or, new this sprint, `GameSession.subscribeToRemoteMatch(matchId)`) now observes every future change to its match document live, with automatic reconnect on disconnect — so whichever future sprint activates `DEALING`/`DASH`/`AUCTION`/etc. as real Firestore writes will have every OTHER seated client already watching for them, with no further sync plumbing to build. See `docs/architecture/MatchSynchronization.md` for the full design, including the documented uid-vs-seat-id limitation that sprint will need to resolve first.
