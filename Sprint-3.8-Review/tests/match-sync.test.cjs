...
(async function () {
  ...
  //   stubs are completely untouched (full regression suites
  //   re-verified separately — see TEST_CHECKLIST.md). submitBid() is
  //   EXCLUDED from this list as of Sprint 3.8 — it is no longer a
  //   stub (see tests/submit-bid.test.cjs for its own dedicated suite);
  //   this list now only asserts about the gameplay methods Sprint 3.8
  //   did NOT touch, per its explicit "only synchronize bidding" scope.
  // ============================================================
