...
// unavoidable wiring.
//
// Sprint 3.6.1 (Normal Dash Scoring Hotfix): Sprint 3.6's version of
// this test deliberately AVOIDED a 0 (Normal Dash) final estimate,
// since that value corrupted scoring into NaN at the time (see
// docs/reviews/MatchFlowIntegration_3.6.md §3). That bug is now fixed
// (docs/reviews/MatchFlowIntegration_3.6.1.md) — this test now
// deliberately INCLUDES a Normal Dash estimate instead of routing
// around it, doubling as live regression coverage for the fix inside
// the same "happy path, complete match" scenario. See
// tests/match-flow-normal-dash-scoring-fix.test.cjs for the dedicated,
// deterministic fix-verification test (the direct descendant of Sprint
// 3.6's bug-reproduction test of the same scenario).
//
...
(function () {
  ...
  // Sprint 3.6.1: the first estimator deliberately submits a Normal
  // Dash (0 tricks) — Sprint 3.6 originally had to AVOID this value
  // here, since it corrupted scoring into NaN (see
  // tests/match-flow-normal-dash-scoring-fix.test.cjs and
  // docs/reviews/MatchFlowIntegration_3.6.1.md for the fix). Now that
  // it's fixed, this "happy path, complete match" test exercises it
  // directly instead of routing around it — a Mixed round (TRICKS +
  // DASH together) is exactly what "Mixed rounds" regression coverage
  // means, and this is the most natural place to prove it.
  var picks = {}; picks[nextCCW(auctionOpener)] = 0;
  var estimateOrder = [];
  var estimateSteps = 0;
  ...
  check("Sprint 3.6.1 fix: this round DOES include a Normal Dash (0) estimate, and it SURVIVED into GameSession.round.estimates " +
    "(all 4 seats present, including the 0) — the exact path Sprint 3.6 had to avoid",
  ...
  check("Every seat received a finite score delta, INCLUDING the Normal Dash seat (proves the 3.6.1 fix live, in a Mixed round)",
