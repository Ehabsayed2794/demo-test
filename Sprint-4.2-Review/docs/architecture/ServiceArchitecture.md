...
- `submitCard(matchId, card) → { matchId, seatId, card, version, cardCount }` — **new in Sprint 4.2.** Deliberately NO `seatId` parameter (unlike `submitBid()`, below) — the acting seat is resolved INTERNALLY from the calling uid via `MatchAdapter.uidToSeat()` (a new, read-only, translation-only dependency edge — see `docs/architecture/EngineAdapter.md`'s Sprint 4.2 section for why this doesn't reintroduce the "MatchService knows the engine" coupling this project has avoided since Sprint 3.4). Runs inside a real Firestore transaction. Does NOT evaluate card legality — validates only a GENERIC shape (a real suit key, a real rank value) and appends `{seatId, card}` to `matches/{matchId}.cardLog` (an append-only log — see that document's own comment for why the schema shape differs from `bids`' single-slot-per-seat map), sets `lastCardSeat`, increments `version` by exactly 1. Rejects with a structured error (`INVALID_ARGUMENT`/`INVALID_CARD_VALUE`/`UNAVAILABLE`/`UNAUTHENTICATED`/`MATCH_ADAPTER_UNAVAILABLE`/`MATCH_NOT_FOUND`/`PERMISSION_DENIED`) on every failure path. See `tests/submit-card.test.cjs` for the full contract and its MOCKED test coverage.
...
- `applyRemoteCard(matchId, matchDoc)` — **new in Sprint 4.2.** Replays every not-yet-applied `matches/{matchId}.cardLog` entry, IN ORDER, through `TableEngine.emit({type:"PlayCard", playerId, card})` — the only call this function makes into any engine file. Gated by a THIRD independent version registry (alongside bid's and turn's — see this document's own EngineAdapter.md cross-reference) plus a per-entry content-level idempotency check, plus a second "how many entries have I replayed" counter (needed because, unlike a bid or a turn, a card delivery can carry MULTIPLE new entries at once). Never mutates Firestore; never calls any engine file other than `TableEngine`.
