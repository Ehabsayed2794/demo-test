...
(async function () {
  ...
  // ============================================================
  // MOCKED — Local card vs. remote card: the ORIGINATING client's own
  // card, applied LOCALLY first (as a real UI would, for instant
  ...
  // TableEngine.emit() directly for its own seat, BEFORE any Firestore
  // round trip completes — exactly as Sprint 3.6's offline flow always has.
  var localEmitResult = TableEngine.emit({ type: "PlayCard", playerId: localSeat, card: localCard });
  check("MOCKED — local card: the local, direct engine call succeeds and is recorded", !localEmitResult.rejected && TableEngine.getState().plays.some(function (p) { return p.playerId === localSeat; }));
  var playsAfterLocal = TableEngine.getState().plays.length;
  // Now the SAME action's echo arrives via Firestore.
  signInAs(seatUidOf(localSeat));
  await MatchService.submitCard("m-localecho", localCard);
  check("MOCKED — local card: the echo of the client's OWN card does not re-execute the engine — play count unchanged (not double-applied)",
    TableEngine.getState().plays.length === playsAfterLocal);
  unsub5();
  finishCurrentTrick();
