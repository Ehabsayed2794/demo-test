...
(function (global) {
  ...
  /** Sprint 4.2, Task 1 (Card Submission): the ONE public API for
   *  submitting a played card. Deliberately `(matchId, card)` — no
   ...
   *  only"), never trusted as a client-supplied claim at all. This is
   *  strictly tighter than submitBid()'s "claim a seat, then verify
   *  ownership" shape — there is no argument here for a caller (buggy
   *  or malicious) to misuse to even ATTEMPT submitting as a seat they
   *  don't own; a uid that owns no seat in this match is rejected
   *  before any write is attempted, full stop.
   *
   *  Uses a real Firestore transaction, exactly like submitBid(). Does
   *  NOT evaluate card legality (Task 1's explicit "must not evaluate
   *  card legality itself") — `card` is stored as an OPAQUE payload
   *  after only the GENERIC shape check above. Does not know or care
   *  about whose turn it is, what's already been played this trick, or
   *  whether this card is even in the calling player's hand — none of
   *  that is this function's job; it only appends one entry to
   *  `cardLog`, sets `lastCardSeat`, and increments `version` by
   *  exactly 1 — "must only persist synchronized state." */
  function submitCard(matchId, card) {
    ...
    return db().runTransaction(function (tx) {
      return tx.get(matchRef).then(function (snap) {
        ...
        var match = snap.data();
        // The ONE call this function makes into MatchAdapter — a
        // read-only translation, never a write, never an engine call.
        // See this file's header comment for the full account of this
        // new, deliberate edge.
        ...
        var cardLog = (match.cardLog || []).slice();
        cardLog.push({ seatId: seatId, card: { suit: card.suit, rank: { v: card.rank.v, s: card.rank.s } } });
        var nextVersion = (match.version || 0) + 1;
        var patch = {
          cardLog: cardLog,
          lastCardSeat: seatId,
        ...
        tx.update(matchRef, patch);
        return { matchId: matchId, seatId: seatId, card: card, version: nextVersion, cardCount: cardLog.length };
      });
    });
  }
