...
> **New in Sprint 3.9 (Engine Adapter Layer), extended in Sprint 4.0 (Online Bidding Synchronization: Authority Layer), extended again in Sprint 4.1 (Turn Authority & Remote Play Validation).** `design-ui/match-adapter.js`.
...
**Depends on** `MatchService` (calls its public `subscribeToMatch()` only — a read; never writes) and `GameSession`/`BiddingEngine` (calls their existing public APIs only — `bootstrapGameSession()`/`applyRemoteBid()`/`applyRemoteTurn()` never add a new field to either). **Is depended on by** nothing yet — no UI screen calls this module. `MatchService` itself has NO dependency on `MatchAdapter`, `GameSession`, or `BiddingEngine`, in either direction — confirmed by this project's forbidden-scope sweep at the end of every sprint that has touched this file.
