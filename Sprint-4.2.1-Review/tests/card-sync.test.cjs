...
/** Sprint 4.2.1: submitCard() now enforces a turn-authority gate
 *  against matches/{matchId}.turn (Task 1) — this test-only helper
 *  keeps that mocked field in sync with whatever the REAL, local
 *  TableEngine currently says, immediately before each submission, so
 *  these end-to-end scenarios (which drive the real engine directly)
 *  don't trip the SAME authority gate tests/submit-card.test.cjs
 *  exercises in isolation elsewhere. Mirrors exactly what a real
 *  turn-sync delivery (Sprint 4.1) would already have done in
 *  production, once that write-back path exists. */
function syncTurnFieldToRealEngine(matchId) {
  STORE[key(matchId)] = Object.assign({}, STORE[key(matchId)], { turn: seatUidOf(TableEngine.getState().turn) });
}
...
(async function () {
  ...
  syncTurnFieldToRealEngine("m-remote");
  ...
  syncTurnFieldToRealEngine("m-dupsnap");
  ...
  syncTurnFieldToRealEngine("m-stale");
  ...
  for (var t = 0; t < 4; t++) {
    ...
    signInAs(uid);
    syncTurnFieldToRealEngine("m-sequence");
  ...
  syncTurnFieldToRealEngine("m-localecho");
  ...
  syncTurnFieldToRealEngine("m-late");
  ...
  syncTurnFieldToRealEngine("m-restart");
