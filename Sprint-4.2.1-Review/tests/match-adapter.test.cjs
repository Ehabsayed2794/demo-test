...
(function () {
  ...
  check("MOCKED — adapter corruption: a cardLog entry missing its own `card` field is skipped as MALFORMED_ENTRY, never crashes, and the count still advances",
    cardCorrupt5.applied === false && cardCorrupt5.results[0].reason === "MALFORMED_ENTRY" && MatchAdapter.getLastAppliedCardCount("m-card-corrupt5") === 1);
