...
**Responsibility:** owns `rooms/{roomId}` — see `RoomLifecycle.md` for the full state machine this implements.

- `createRoom(hostUid) → roomId` (join code)
- `joinRoom(roomId, uid) → seatIndex` (must use a transaction — see `RoomLifecycle.md`'s race-condition note)
- `setReady(roomId, uid, ready: boolean)`
- `leaveRoom(roomId, uid)`
- `transferHost(roomId, newHostUid)` (transaction-guarded, per `RoomLifecycle.md`)
- `closeRoom(roomId)`
