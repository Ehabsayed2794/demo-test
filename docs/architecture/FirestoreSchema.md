...
- **Purpose:** A Play-with-Friends lobby prior to a match starting — join-by-code, ready-up, host controls.
- **Document ID:** a short human-typeable code (e.g. 6 chars, uppercase alphanumeric, collision-checked at creation) — this **is** the join code, not a separate field, so "join by code" is a direct `get()` by ID, not a query.
- **Fields:**
  - `hostUid: string`
  - `seats: [{ uid: string|null, ready: boolean, isAI: boolean }]` (length 4, mirrors `session.room.seats` today)
  - `status: "waiting" | "ready" | "starting" | "closed"`
  - `matchId: string | null` (set once the match is created — see `MatchLifecycle.md`)
  - `createdAt: timestamp`, `expiresAt: timestamp` (Firestore native TTL field — see `RoomLifecycle.md`)
- **Relationships:** `seats[].uid` → `players/{uid}`; `matchId` → `matches/{matchId}`.
- **Read frequency:** Medium — every seated player listens live (`onSnapshot`) while in the room screen.
- **Write frequency:** Low-medium — one write per join/ready-toggle/leave, bursty at room formation, silent afterward.
- **Security requirements:** any authenticated user may read a room (needed to preview before joining); only a seated player may write, and only to their own seat's `ready` field or to leave (clear their own seat) — never another seat. Only `hostUid` may write `status` transitions or kick/close. See `SecurityArchitecture.md`.
