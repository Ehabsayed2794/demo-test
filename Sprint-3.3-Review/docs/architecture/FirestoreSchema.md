...
- **Fields (current, actual):**
  ...
  - `status: "waiting" | "closed"` — `"waiting"` from creation; `"closed"` once the last player leaves. (`"ready"`/`"starting"` from the original draft still do not exist — an automatic status transition when all members are ready is explicitly out of scope; see `RoomLifecycle.md`'s "Ready State" section.)
...
- **Fields removed/deprecated — DO NOT implement against these:**
  ...
  - ~~`matchId`~~, ~~`expiresAt`~~ → still not yet implemented; no match-creation or TTL-cleanup code exists yet (the latter is Sprint 3.2.5 Architecture Audit finding F8, still open — see `RoomLifecycle.md`'s "Room Expiration" section).
