...
function isValidCardSubmission(oldData, newData, requestAuthUid) {
  ...
  if (!("seats" in oldData) || !("cardLog" in oldData) || !("version" in oldData) || !("turn" in oldData) || !("cardPhase" in oldData)) return false;
...
function cardSubmissionWith(card) {
  return Object.assign({}, matchAfterCreate42, {
    cardLog: [{ seatId: "p1", card: card }], version: 2, lastCardSeat: "p1", turn: "userC", cardPhase: "PLAY"
  });
}
